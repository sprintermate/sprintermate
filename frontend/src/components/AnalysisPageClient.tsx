...
import AgentPromptsModal from './AgentPromptsModal';
...
/** Detect whether the content is legacy HTML or new Markdown. */
...
export default function AnalysisPageClient({ projects, locale }: Props) {
  ...
  const sendMessage = async () => {
    ...
    if (selectedAgentId) formData.append('agentPromptId', selectedAgentId);
    ...
    const tempUserMsg: Message = {
      ...
      attachments: selectedFile ? JSON.stringify([{ name: selectedFile.name, type: selectedFile.type }]) : null,
    ...
    try {
      const res = await fetch(`${BACKEND}/api/analysis/sessions/${sessionId}/messages`, {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
      if (res.ok || res.status === 500) {
        ...
        setMessages(prev => {
          ...
          const result = [...filtered, data.userMessage];
          if (data.assistantMessage) result.push(data.assistantMessage);
          return result;
        });
        setSessions(prev => prev.map(s =>
          s.id === sessionId
            ? { ...s, title: data.userMessage?.content?.slice(0, 80) || s.title, updated_at: new Date().toISOString() }
            : s
        ));
      }
    } catch { /* ignore */ }
  ...
  return (
    ...
    <div className="flex h-full bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950">
        ...
        <aside className="w-72 border-r border-white/[0.06] flex flex-col bg-slate-950/60 flex-shrink-0">
          ...
          <div className="p-3 border-b border-white/[0.06] flex items-center gap-2">
            <button
              ...
              className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-slate-500 hover:text-slate-300 hover:bg-white/[0.06] transition-all flex-shrink-0"
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium rounded-xl transition-all shadow-lg shadow-violet-500/20 hover:shadow-violet-500/30"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
            {sessionsLoading ? (
              ...
              <div className="text-center py-12 px-4">
                ...
                <p className="text-xs text-slate-600">{t('noSessions')}</p>
              </div>
            ) : (
              sessions.map(session => (
                ...
                >
                  ...
                  {editingSessionId === session.id ? (
                    <input
                      className="flex-1 text-sm bg-slate-800 border border-slate-600 rounded-lg px-2 py-0.5 text-white focus:outline-none focus:border-violet-500"
                      value={editTitle}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Config bar */}
        <div className="flex items-center gap-4 px-5 py-2.5 border-b border-white/[0.06] bg-slate-950/40 backdrop-blur-sm flex-shrink-0">
          {/* Sidebar toggle (when hidden) */}
          {!sidebarOpen && (
            <button
              onClick={() => setSidebarOpen(true)}
              className="w-8 h-8 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-slate-500 hover:text-slate-300 transition-all flex-shrink-0"
              title="Open sidebar"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={e => { setSelectedProjectId(e.target.value); setSelectedRepoIds([]); setRepoSearch(''); }}
              className="bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-1.5 text-sm text-slate-300 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/20 max-w-[220px] transition-all"
            >
          ...
          {selectedProjectId && (
            <div className="relative flex items-center gap-2" ref={repoDropdownRef}>
              ...
              <button
                ...
                className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-1.5 text-sm text-slate-300 hover:border-violet-500/30 transition-all min-w-[180px] max-w-[320px]"
              >
                {reposLoading ? (
                  <span className="flex items-center gap-1.5">
                    <div className="w-3 h-3 border-2 border-violet-400 border-t-transparent rounded-full animate-spin" />
                    <span className="text-slate-500 text-xs">{t('repoLoading')}</span>
                  </span>
                ) : selectedRepoIds.length > 0 ? (
                  <span className="flex items-center gap-1.5 truncate">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-md bg-violet-500/20 text-violet-300 text-xs font-bold">{selectedRepoIds.length}</span>
                    <span className="truncate text-xs">{t('selectRepos').toLowerCase()}</span>
                  </span>
                ) : (
                  <span className="text-slate-500 text-xs">{t('searchRepos')}</span>
                )}
                <svg className={`w-3.5 h-3.5 flex-shrink-0 ml-auto transition-transform ${repoDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="absolute top-full left-0 mt-1.5 z-50 w-96 bg-slate-900 border border-white/[0.1] rounded-xl shadow-2xl shadow-black/40 overflow-hidden">
                  <div className="p-2 border-b border-white/[0.06]">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg pl-8 pr-3 py-1.5 text-sm text-slate-300 placeholder-slate-600 focus:outline-none focus:border-violet-500/30"
                        autoFocus
                      />
                    </div>
                  </div>
                      ...
                      filteredRepos.map(repo => (
                        <label key={repo.id} className="flex items-center gap-3 px-3 py-2 hover:bg-white/[0.04] cursor-pointer transition-colors">
                          <input
                            ...
                            className="w-4 h-4 rounded border-slate-600 text-violet-500 focus:ring-violet-500/30 bg-transparent"
                          />
                          <div className="flex-1 min-w-0">
                            <div className="text-sm text-slate-300 truncate">{repo.name}</div>
                            {repo.defaultBranch && (
                              <div className="text-[11px] text-slate-600">{repo.defaultBranch.replace('refs/heads/', '')}</div>
                            )}
                          </div>
                        </label>
                      ))
                  ...
                  {selectedRepoIds.length > 0 && (
                    <div className="px-3 py-2 border-t border-white/[0.06] flex items-center justify-between">
                      <span className="text-xs text-slate-500">{selectedRepoIds.length} selected</span>
                      <button onClick={() => setSelectedRepoIds([])} className="text-xs text-red-400 hover:text-red-300 transition-colors">
                        Clear all
                      </button>
                    </div>
                  )}
          ...
          {selectedRepoIds.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap max-w-[400px]">
              {selectedRepoIds.slice(0, 3).map(id => {
                ...
                return (
                  <span key={id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-violet-500/10 text-violet-300 text-xs border border-violet-500/20">
                    ...
                    <button onClick={() => toggleRepo(id)} className="hover:text-red-400 transition-colors">
              ...
              {selectedRepoIds.length > 3 && (
                <span className="text-xs text-slate-500">+{selectedRepoIds.length - 3}</span>
              )}
            </div>
          )}
          ...
          <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
            ...
            <select
              ...
              className="bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-1.5 text-sm text-slate-300 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/20 max-w-[180px] transition-all"
            >
              <option value="" className="bg-slate-900">{t('agentDefault')}</option>
              {agents.map(a => (
            ...
            <button
              ...
              className="w-7 h-7 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-slate-500 hover:text-violet-400 hover:border-violet-500/30 transition-all flex-shrink-0"
        ...
        <div className="flex-1 overflow-y-auto">
          {messages.length === 0 && !loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              ...
              <h3 className="text-lg font-semibold text-white mb-2">{t('noSessions')}</h3>
            ...
            <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
                ...
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[90%] ${msg.role === 'user' ? '' : 'w-full'}`}>
                    {msg.role === 'user' ? (
                      ...
                      /* Assistant bubble — auto-detect MD vs HTML */
                        ...
                        <div className="px-5 py-4">
                          {isMarkdown(msg.content) ? (
                            <AnalysisMDOutput
                              ...
                              filename={`analysis-${msg.created_at.slice(0, 10)}`}
                            />
                          ) : (
                    ...
                    {msg.role === 'user' && (
                      <div className="text-[10px] mt-1 text-right text-slate-600">
                        {new Date(msg.created_at).toLocaleTimeString()}
                      </div>
                    )}
                  </div>
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-white/[0.03] backdrop-blur-sm rounded-2xl rounded-bl-md px-5 py-4 border border-white/[0.06]">
          ...
          <div className="max-w-4xl mx-auto">
            {selectedFile && (
              <div className="flex items-center gap-2 mb-3">
                <div className="flex items-center gap-2 bg-violet-500/10 border border-violet-500/20 text-violet-300 rounded-lg px-3 py-1.5 text-xs font-medium">
                  ...
                  {selectedFile.name}
                  <button onClick={() => setSelectedFile(null)} className="ml-1 hover:text-red-400 transition-colors">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </button>
            ...
            <div className="flex items-end gap-3">
              ...
              <button
                ...
                className="flex-shrink-0 w-10 h-10 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-slate-400 hover:text-violet-400 hover:border-violet-500/30 hover:bg-violet-500/5 transition-all"
                title={t('attachFile')}
              >
              ...
              <div className="flex-1">
                <textarea
                  ...
                  rows={1}
                  className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-violet-500/40 focus:ring-1 focus:ring-violet-500/20 resize-none transition-all"
                  disabled={loading}
                />
              </div>
              ...
              <button
                ...
                className="flex-shrink-0 w-10 h-10 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:bg-white/[0.04] disabled:border disabled:border-white/[0.08] text-white disabled:text-slate-600 transition-all shadow-lg shadow-violet-500/20 hover:shadow-violet-500/30 disabled:shadow-none flex items-center justify-center"
                title={t('sendMessage')}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
