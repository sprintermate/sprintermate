      ...
      {activeTab === 'rooms' && (
        <section>
          <div className="flex flex-wrap gap-3 mb-8">
            ...
            <button
              disabled
              className="px-4 py-2 rounded-lg border border-slate-700 text-slate-400 text-sm font-medium cursor-not-allowed opacity-60"
            >
              Join by Code
            </button>
          </div>
          ...
          {rooms.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {rooms.map(room => (
                <div
                  key={room.id}
                  className="relative p-4 rounded-xl border border-slate-800 hover:border-slate-700 bg-slate-900/50 hover:bg-slate-900 transition-colors group"
                >
                  <a href={`/${locale}/room/${room.code}`} className="block">
                    <div className="flex items-center justify-between mb-3 pr-7">
                      <span className="font-mono text-lg font-bold text-indigo-400 tracking-widest">
                        {room.code}
                      </span>
                  ...
                  <button
                    ...
                    title="Delete room"
                    className="absolute top-3 right-3 p-1.5 rounded-lg text-slate-600 hover:text-red-400 hover:bg-red-500/10 transition-colors disabled:opacity-40"
                  >
                  ...
                  {editingId === project.id ? (
                    ...
                    <div className="p-4 space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="block text-xs text-slate-500 mb-1">ADO URL</label>
                          <input
                            type="url"
                      ...
                      <div className="border-t border-slate-800 pt-4 space-y-3">
                        <div>
                          <p className="text-xs font-medium text-slate-400">Reference story point examples</p>
                          <p className="text-xs text-slate-600 mt-0.5">Example work items with known story points to help the team calibrate estimates.</p>
                        </div>
                        ...
                        {scoresLoading ? (
                          ...
                          <>
                            {editScores.length > 0 && (
                              <div className="space-y-2">
                                {editScores.map(score => (
                                  <div key={score.tempId} className="bg-slate-800/60 border border-slate-700 rounded-lg p-2.5 space-y-2">
                                    <div className="flex items-center gap-2">
                                      <input
                                        ...
                                        className="flex-1 bg-slate-800 border border-slate-700 rounded-md px-2.5 py-1.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500"
                                      />
                                      <input
                                        type="number"
                                        ...
                                        min="0"
                                        step="0.5"
                                        className="w-16 bg-slate-800 border border-slate-700 rounded-md px-2.5 py-1.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500 text-center"
                                      />
                                      <button
                                    ...
                                    <input
                                      ...
                                      className="w-full bg-slate-800 border border-slate-700 rounded-md px-2.5 py-1.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500"
                                    />
                                  </div>
                                ))}
                              </div>
                            ...
                            <button
                              ...
                              className="w-full py-1.5 rounded-lg border border-dashed border-slate-700 hover:border-indigo-500 text-slate-500 hover:text-indigo-400 text-xs font-medium transition-colors"
                            >
                              + Add reference item
                            </button>
                          </>
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium disabled:opacity-50 transition-colors"
                        >
                    ...
                    <div className="flex items-center justify-between p-4">
                      ...
                      <div className="flex items-center gap-2 ml-4 shrink-0">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg border border-slate-700 hover:border-slate-600 text-slate-400 hover:text-white text-xs font-medium transition-colors"
                        >
                          Edit
                        </button>
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg border border-red-900/50 hover:border-red-700 text-red-500 hover:text-red-400 text-xs font-medium transition-colors disabled:opacity-50"
                        >
