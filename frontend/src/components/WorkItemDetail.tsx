...
/** Pastel gradient per Fibonacci value — cool for small estimates, warm for large ones, each blending into its neighbor. */
const CARD_COLORS: Record<number, string> = {
  1: 'bg-gradient-to-br from-sky-100 to-cyan-100 text-sky-700 border-sky-200 dark:from-sky-900/40 dark:to-cyan-900/30 dark:text-sky-200 dark:border-sky-700/40',
  2: 'bg-gradient-to-br from-cyan-100 to-teal-100 text-teal-700 border-teal-200 dark:from-cyan-900/40 dark:to-teal-900/30 dark:text-teal-200 dark:border-teal-700/40',
  3: 'bg-gradient-to-br from-teal-100 to-emerald-100 text-emerald-700 border-emerald-200 dark:from-teal-900/40 dark:to-emerald-900/30 dark:text-emerald-200 dark:border-emerald-700/40',
  5: 'bg-gradient-to-br from-emerald-100 to-lime-100 text-lime-700 border-lime-200 dark:from-emerald-900/40 dark:to-lime-900/30 dark:text-lime-200 dark:border-lime-700/40',
  8: 'bg-gradient-to-br from-lime-100 to-yellow-100 text-yellow-700 border-yellow-200 dark:from-lime-900/40 dark:to-yellow-900/30 dark:text-yellow-200 dark:border-yellow-700/40',
  13: 'bg-gradient-to-br from-yellow-100 to-amber-100 text-amber-700 border-amber-200 dark:from-yellow-900/40 dark:to-amber-900/30 dark:text-amber-200 dark:border-amber-700/40',
  21: 'bg-gradient-to-br from-amber-100 to-orange-100 text-orange-700 border-orange-200 dark:from-amber-900/40 dark:to-orange-900/30 dark:text-orange-200 dark:border-orange-700/40',
  34: 'bg-gradient-to-br from-orange-100 to-rose-100 text-rose-700 border-rose-200 dark:from-orange-900/40 dark:to-rose-900/30 dark:text-rose-200 dark:border-rose-700/40',
  55: 'bg-gradient-to-br from-rose-100 to-fuchsia-100 text-fuchsia-700 border-fuchsia-200 dark:from-rose-900/40 dark:to-fuchsia-900/30 dark:text-fuchsia-200 dark:border-fuchsia-700/40',
};
...
export default function WorkItemDetail({
  ...
  // ADO tags — seeded from the list payload, refreshed with the full detail fetch
    ...
    <div className="flex flex-col gap-0 flex-1 min-h-0">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              {aiLoading ? (
                ...
                <>
                  ...
                  <span>{t('startScoringWithAI')}</span>
                </>
              )}
            </button>
          )}
          {isModerator && scoringActive && !revealed && (
            <button
              onClick={onReveal}
              disabled={!aiEstimate && !savedAiEstimate && votedCount === 0}
      ...
      <div className={`flex-1 min-h-0 ${scoringActive ? 'grid grid-cols-12 gap-6' : 'flex flex-col'}`}>
        ...
        <div className={`flex flex-col gap-5 overflow-y-auto min-h-0 ${scoringActive ? 'col-span-8' : 'w-full'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {tags.length > 0 && (
                  <div className="flex flex-wrap items-center gap-1.5 mt-2">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-slate-500 mr-0.5">
                      {t('sectionTags')}
                    </span>
                    {tags.map((tag) => <TagChip key={tag} tag={tag} className="text-xs px-2.5 py-1" />)}
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Batch estimation in-progress indicator (no score shown) */}
          {!scoringActive && isBeingBatchEstimated && (
            <div className="rounded-xl bg-gradient-to-br from-teal-50 to-cyan-50 border border-teal-200 dark:from-violet-900/40 dark:to-indigo-900/20 dark:border-violet-500/30 p-5">
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 border-2 border-violet-400/60 border-t-violet-400 rounded-full animate-spin shrink-0" />
                <p className="text-xs font-semibold text-teal-700 dark:text-violet-300">✨ {t('analyzing')}</p>
              </div>
            </div>
          )}
        </div>
                  ...
                  <button
                    ...
                    className={`h-10 rounded-lg text-sm font-bold transition-all border ${
                      myScore === n
                        ...
                        : `${CARD_COLORS[n]} hover:scale-[1.04] hover:shadow-sm`
                    }`}
                  >
                    {n}
                  </button>
              ...
              <div className="rounded-xl bg-gradient-to-br from-teal-50 to-cyan-50 border border-teal-200 dark:from-violet-900/40 dark:to-indigo-900/20 dark:border-violet-500/30 p-4">
                ...
                {isBeingBatchEstimated && !displayEstimate ? (
                  <div className="flex items-center justify-center gap-2 py-4">
                    <div className="w-4 h-4 border-2 border-violet-400/60 border-t-violet-400 rounded-full animate-spin" />
                    <span className="text-xs text-teal-700 dark:text-violet-300">{t('analyzing')}</span>
                  </div>
                ) : displayEstimate && (
                  <>
                    {/* SP Value */}
                    <div className="flex items-baseline justify-center gap-1 mb-3">
                      <span className="text-4xl font-bold text-gray-900 dark:text-white">{displayEstimate['story-point']}</span>
                    ...
                    <div className="mb-3">
                      ...
                      <p className="text-xs text-gray-600 dark:text-slate-300 leading-relaxed">{displayEstimate.analysis}</p>
                    </div>

                    {/* Similar Items */}
                    {displayEstimate['similar-items'].length > 0 && (
                      <div>
                        ...
                        <ul className="space-y-1.5">
                          {displayEstimate['similar-items'].map((item, i) => (
                ...
                <ul className="space-y-2">
                  ...
                  {scoringActive && (aiLoading || isBeingBatchEstimated || displayEstimate) && (
                    <li className="flex items-center gap-3">
                      ...
                      <span className="flex-1 text-sm text-violet-700 dark:text-violet-300 truncate font-medium">AI</span>
                      {(aiLoading || isBeingBatchEstimated) && !displayEstimate ? (
                        <span className="w-8 h-8 flex items-center justify-center">
                          <div className="w-3.5 h-3.5 border-2 border-violet-400/60 border-t-violet-400 rounded-full animate-spin" />
                        </span>
                      ) : revealed && displayEstimate ? (
                        <span className="w-8 h-8 flex items-center justify-center rounded-lg bg-violet-50 border border-violet-200 text-violet-700 dark:bg-violet-500/20 dark:border-violet-500/30 dark:text-violet-300 font-bold text-sm">
                          {displayEstimate['story-point']}
                        </span>
                      ) : displayEstimate ? (
