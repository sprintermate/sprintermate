...
export default function AnalysisChat({ projects, onClose }: AnalysisChatProps) {
  ...
  const deleteSession = async (id: string) => {
    ...
    try {
      await fetch(`${BACKEND}/api/analysis/sessions/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  // ─── Render ─────────────────────────────────────────────────────────────
  return (
    <div className="fixed inset-0 z-50 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 flex flex-col">

      {/* ── Header Bar ──────────────────────────────────────────────── */}
      <header className="h-14 flex items-center gap-3 px-4 border-b border-white/[0.06] bg-slate-950/80 backdrop-blur-xl flex-shrink-0">
        <button onClick={onClose} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-white/[0.06] transition-all">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold text-white">{t('tabTitle')}</h1>
        </div>
        <div className="flex-1" />
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/[0.06] transition-all" title="Toggle sidebar">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3.75 6.75h16.5M3.75 12h16.5M12 17.25h8.25" /></svg>
        </button>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <aside className="w-72 border-r border-white/[0.06] flex flex-col bg-slate-950/60 flex-shrink-0">
            ...
            <div className="p-3 border-b border-white/[0.06]">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium rounded-xl transition-all shadow-lg shadow-violet-500/20 hover:shadow-violet-500/30"
              >
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {/* Config bar — Project & Repo selection */}
          <div className="flex items-center gap-4 px-5 py-3 border-b border-white/[0.06] bg-slate-950/40 backdrop-blur-sm flex-shrink-0">
            {/* Project selector */}
            <div className="flex items-center gap-2">
                ...
                >
                  {reposLoading ? (
                    <span className="flex items-center gap-1.5">
                      ...
                      <span className="text-slate-500">{t('repoLoading')}</span>
                    </span>
                    ...
                    <span className="flex items-center gap-1.5 truncate">
                      ...
                      <span className="truncate">{t('selectRepos').toLowerCase()}</span>
                    </span>
                  ) : (
                    <span className="text-slate-500">{t('searchRepos')}</span>
                  )}
                    ...
                    <div className="max-h-72 overflow-y-auto">
                      {filteredRepos.length === 0 ? (
                        ...
                        filteredRepos.map(repo => (
                          <label
                            key={repo.id}
                            className="flex items-center gap-3 px-3 py-2 hover:bg-white/[0.04] cursor-pointer transition-colors"
                          >
                            <input
                              type="checkbox"
