...
async function getSprints(cookieHeader: string, projectId: string) {
  try {
    const res = await fetch(
      `${process.env.BACKEND_URL}/api/projects/${projectId}/sprints`,
      {
        headers: { Cookie: cookieHeader },
        cache: 'no-store',
      }
...
export default async function ProjectMetricsPage({ params }: Props) {
  ...
  const [project, sprints] = await Promise.all([
    ...
    getSprints(cookieHeader, projectId),
  ]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400">
              <span>/</span>
              <span className="font-medium text-indigo-600 dark:text-indigo-400">{project.name}</span>
              <span>/</span>
              <span className="font-medium text-gray-700 dark:text-slate-300">Metrics</span>
            </div>
          </div>
